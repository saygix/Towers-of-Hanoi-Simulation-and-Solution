...
var hanoi = (function() {
    ...
    var solveHanoi = function(numDisks, fromPeg, toPeg) {
        ...
        // move numDisks - 1 disks from fromPeg to sparePeg
        solveHanoi(numDisks - 1, fromPeg, sparePeg);
        
        // move the largest disk from fromPeg to toPeg
        hanoi.moveDisk(fromPeg, toPeg);
        
        // move the remaining disks from sparePeg to toPeg
        solveHanoi(numDisks - 1, sparePeg, toPeg);
    };
    
    // Call solveHanoi to solve the puzzle for 5 disks from peg A to peg B
    solveHanoi(5, "A", "B");
    
    // Check if the puzzle is solved on peg B
    var isSolved = hanoi.isSolved("B");
    
    // Output the result using println
    if (isSolved) {
        println("Puzzle is solved!");
    } else {
        println("Puzzle is not solved.");
    }
